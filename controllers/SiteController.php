...
class SiteController extends Controller
{
    ...
    {
        $query = new Query();
        $sapChieu = $query->select(['id','attributes','status','slug'])->from('phim')->where(['status' => 1])->orderBy(['created_at' => SORT_DESC])->limit(6)->all();
        $dangChieu = $query->select(['id','attributes','status','slug'])->from('phim')->where(['status' => 2])->orderBy(['created_at' => SORT_DESC])->limit(6)->all();
